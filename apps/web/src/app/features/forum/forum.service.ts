...
export class ForumService {
    ...
    getThread(threadId: number) {
        return firstValueFrom(
            this.http.get<ForumThreadDetail>(`/api/forum/threads/${threadId}`)
        );
    }

    createThread(restaurantId: string, title: string, content: string) {
        return firstValueFrom(
            this.http.post<ForumThreadListItem>(`/api/forum/restaurants/${restaurantId}/threads`, { title, content })
        );
    }
    ...
    closeThread(threadId: number) {
        return firstValueFrom(
            this.http.patch(`/api/forum/threads/${threadId}/close`, {})
        );
    }

    deleteThread(threadId: number) {
        return firstValueFrom(
            this.http.delete(`/api/forum/threads/${threadId}`)
        );
    }
